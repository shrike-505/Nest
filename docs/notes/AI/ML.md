...
- Classification Error 分类错误率 = $\frac{1}{m} \sum_{i=1}^{m} I(y_i \neq \hat{y}_i)$
- Mean Squared Error 均方误差 = $\frac{1}{m} \sum_{i=1}^{m} (y_i - \hat{y}_i)^2$
- Cross Entropy 交叉熵 = $\frac{1}{N} \sum_{i} - [y_i \log \hat{y}_i + (1 - y_i) \log (1 - \hat{y}_i)]$
...
!!! example "一般问题的计算流程"
    ...
    一般题目就是：根据给定数据集学习一个朴素贝叶斯分类器，判断特征为（属性1 = 属性值1、属性2 = 属性值2、...、属性d = 属性值d）的新样本属于哪个类别（类别1、类别2、...、类别N）

    1. 计算先验概率 $P(类别 1), P(类别 2), ..., P(类别 N)$
    2. 计算条件概率 $P(属性 1 = 属性值 1 | 类别 i), P(属性 2 = 属性值 2 | 类别 i), ..., P(属性 d = 属性值 d | 类别 i)$，$i = 1, 2, ..., N$
    3. 计算后验概率 $P(类别 i | 属性 1 = 属性值 1, 属性 2 = 属性值 2, ..., 属性 d = 属性值 d)$，$i = 1, 2, ..., N$
       1. $P(类别 i | 属性 1 = 属性值 1, 属性 2 = 属性值 2, ..., 属性 d = 属性值 d) \propto P(类别 i) \prod_{j=1}^{d} P(属性 j = 属性值 j | 类别 i)$
    4. 选择后验概率最大的类别作为预测类别
...
利用距离样本最近的 K 个训练样本的类别来决定测试样本的类别，可见关键在于 **k 值选取**与**距离计算**
...
- MDS (Multidimensional Scaling, MDS)
    ...
    - ![MDS](./assets/ML20.png)
- PCA
    - 求均值向量和中心化矩阵：中心化的矩阵 $X'$ 是将每个样本点减去平均值向量 $\bar{x}$。
    - 求主成分的单位向量
        - 求协方差矩阵 $C = \frac{1}{n} X' (X')^T$
        - 求协方差矩阵 $C$ 的特征值和特征向量
        - 将特征向量归一化
...
对于线性回归模型，添加正则项（L2范数）后，其优化目标是

$$
\min \sum_{i=1}^{m} (y_i - w \cdot x_i - b)^2 + \lambda ||w||_2^2
$$

将 L2 范数替换为 L1 范数，得到 Lasso 回归

$$
\min \sum_{i=1}^{m} (y_i - w \cdot x_i - b)^2 + \lambda ||w||_1
$$

更易获得稀疏解，因此更适合特征选择，如下图所示

![L1 Lasso](./assets/ML23.png)

### 字典学习

为稠密表示的数据集样本找到合适的字典，将其转化为稀疏表达，后者有**文本数据线性可分**，**存储高效**的优点。

给定数据集 ${x_1, x_2, ..., x_m}, x_i \in \mathbb{R}^{n \times k}$，学习目标是 $\bold{B} \in \mathbb{R}^{d \times k}$ 和样本稀疏表示 $\alpha_i \in \mathbb{R}^{k}$，$k$ 为字典词汇量，由用户指定，字典优化形式为

$$
\min_{\bold{B}, \alpha_i} \sum_{i=1}^{m} ||x_i - \bold{B} \alpha_i||_2^2 + \lambda \sum_{i=1}^{m} ||\alpha_i||_1
$$
...
数据集包含少量带标记样本和大量未标记样本，让学习器不依赖外界交互，自动利用未标记样本提升学习性能。
...
给定有标记样本集和未标记样本集。假设所有样本独立同分布，且都是由同一个模型生成的。

### 半监督 SVM

在考虑未标记样本后， S3VM试图找到能将两类有标记样本分开，且穿过**数据低密度**区域的划分超平面

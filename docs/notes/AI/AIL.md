...
??? note "课后作业"
    - [HW1](./assets/第一次课后练习.pdf)
    - [HW2](./assets/第二次课后练习.pdf)
    - [HW3](./assets/第三次课后练习.pdf)
    - [HW4](./assets/第四次课后练习.pdf)
    - [HW5](./assets/第五次课后练习.pdf)
    - [HW6](./assets/第六次课后练习.pdf)
    - [HW7](./assets/第七次课后练习.pdf)
    - [HW8](./assets/第八次课后练习.pdf)
    - [HW9](./assets/第九次课后练习.pdf)
    - [HW10](./assets/第十次课后练习.pdf)
    - [HW11](./assets/第十一次课后练习.pdf)
    - [HW12](./assets/第十二次课后练习.pdf)
    - [HW13](./assets/第13次课后练习.pdf)
    - [HW14](./assets/第14次课后练习.pdf)
    - [HW15](./assets/第15次课后练习.pdf)
    - [附加题](./assets/附加题.pdf)
...
合取起来的子句记为子句集合的元素（放在大括号里），析取的文字放在中括号里。
...
我们还是要判断一个子句集合是否可满足，运用上面的消解原理，可能会出现无限循环的情况（这里针对一阶逻辑而言），但命题逻辑总是可以判断可满足性的，于是某些情况下需要考虑把一阶逻辑规约到命题逻辑，这就需要赫布兰德域/基底的概念。

!!! definition "赫布兰德域"
    设 $S$ 是一个字句集合，则 $H = \bigcup_{i=0}^{\infty} H_i$ 是 $S$ 的赫布兰德域，其中 $H_i$ 这样递归定义：

    $H_0 = \{a\}$ 如果 $S$ 中无任何常元，否则 $H_0 = \{c|c 为 S 中出现的常元\}$；
...
相比一阶逻辑的语义（探求可满足性），缺省理论的语义指的是可以接受或相信哪些语句。被接受或相信的语句集合称为**外延**。

!!! definition "缺省规则的应用"
    给定两个句子集合 $Φ$ 和 $Ψ$，以及一条缺省规则 $𝛿 = 𝜑 : 𝜓_1,..., 𝜓_𝑛 / 𝜒$，我们说在 $Φ$ 下把 $𝛿$ 应用于 $Ψ$，意指：如果 $𝜑 ∈ Ψ$，而且 $¬𝜓1 ∉ Φ, . . . , ¬𝜓𝑛 ∉ Φ$，那么 $𝜒 ∈ Ψ$。当 $Φ = Ψ$ 时，我们也说把 $𝛿$ 应用于 $Ψ$。

这里有一个神秘算子 $\Gamma$ 的定义（TBD）

!!! definition "外延的不动点"
    句子集合 $𝐸$ 是缺省理论 $𝑇 = ⟨𝑊, 𝐷⟩$ 的一个外延当且仅当 $𝐸$ 是算子 $Γ$ 的一个不动点，即 $Γ(𝐸) = 𝐸$。

试一下就会知道这种方法来求外延十分十分麻烦，于是有一个准归纳的方法

!!! note "准归纳法"
    设 $𝐸$ 为一阶命题集，$𝑇 =< 𝑊, 𝐷 >$ 为一闭的缺省理论。递归定义 $𝐸_𝑖 (𝑖 = 1, 2, . . . )$ 如下：

    - $𝐸_0 = 𝑊$
    - $E_{i+1} = Th(E_i) \cup \{𝜒| 𝜑 : 𝜓_1,..., 𝜓_𝑛 / 𝜒 \in D, 𝜑 ∈ E_i, ¬𝜓_1,..., ¬𝜓_𝑛 ∉ E\}$
...
我们希望扩大可相容外延的范围，我们发现其不仅可以防御**自身内部**的每个论证，也可以防御整个论证集合中剩下部分的某些论证，不如把这些被防御的论证也加入外延（由上面的定理，加入后的外延仍然是可相容的）。

$AF = <AR, attacks>$ 的特征函数 $F_{AF}(S)=\{\alpha | S 可防御 \alpha\}$

当 $𝐹_{𝐴𝐹}(𝑆) = 𝑆$ 时，我们称 $𝑆$ 是 $𝐹_{𝐴𝐹}$ 的不动点。这时，集合 𝑆 中包含了所有 𝑆 可防御的论证。我们把这样的集合称为完全外延。

可理解为完全外延 = 可相容外延 and 不动点

特征函数不动点不要求是无冲突的，但是完全外延必须是无冲突的（因为其首先是可相容的）。
...
例如说对于三个外延 $\{\}, \{b\}, \{a, c\}$，其中 $\{\}$ 是基外延，$\{b\}$ 和 $\{a, c\}$ 是优先外延。
...
Nieves: 主要用于计算**优先外延**。映射产生一个析取逻辑程序，该程序定义一个谓词 def，其中 $def(𝑎)$ 可以被读作” 论证 $𝑎$ 被击败”。
...
接着在**完全语义**下，（与论辩框架无关）依据论证标记得到下列句子集合：
...
**稳定语义**下，论证不能为 undec，则逻辑程序 $P^{st}_{Wakaki} = P^{co}_{Wakaki} \cup \{\leftarrow undec(X)\}$

Egly: TBD

## 易错题

[易错题](./assets/AIL易错题.pdf)